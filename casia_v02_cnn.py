...
class CNN(nn.Module):
    def __init__(self, output_dim, embedding_dim):
        ...
        self.conv1 = nn.Sequential(nn.Conv1d(4, 32, 7, stride=2, padding=3), nn.LeakyReLU())
        self.conv2 = nn.Sequential(nn.Conv1d(32, 64, 7, stride=2, padding=3), nn.LeakyReLU())
        self.conv3 = nn.Sequential(nn.Conv1d(64, 128, 7, stride=2, padding=3), nn.LeakyReLU())
        self.conv4 = nn.Sequential(nn.Conv1d(128, 64, 7, stride=2, padding=3), nn.LeakyReLU())
        self.conv5 = nn.Sequential(nn.Conv1d(64, 32, 7, stride=1, padding=3), nn.LeakyReLU())
        self.conv6 = nn.Sequential(nn.Conv1d(32, 16, 7, stride=1, padding=3), nn.LeakyReLU(), nn.Dropout1d(0.3))

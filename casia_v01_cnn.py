...
input_dim = 300    # 每个汉字坐标总数，如果不够需要使用0进行填充
...
class CNN(nn.Module):
    def __init__(self, output_dim, embedding_dim):
        ...
        # layer1
        # conv1     in_channel=4, out_channel=64, kernel_size=5, stride=1, padding=2
        # conv1                4x256       64x256
        # pool1     output_size=128
        # pool1                64x256      64x128

        # layer2
        # conv2     in_channel=64, out_channel=64, kernel_size=5, stride=1, padding=2
        # conv2                64x128      64x128
        
        self.conv1 = nn.Sequential(nn.Conv1d(4, 64, 5, stride=1, padding=2), nn.AdaptiveMaxPool1d(output_size=128), nn.Dropout1d(0.2))
        self.conv2 = nn.Sequential(nn.Conv1d(64, 128, 5, stride=1, padding=2), nn.Dropout1d(0.2), nn.ReLU())
        self.conv3 = nn.Sequential(nn.Conv1d(128, 256, 5, stride=1, padding=2), nn.Dropout1d(0.2), nn.ReLU())
        self.conv4 = nn.Sequential(nn.Conv1d(256, 64, 5, stride=1, padding=2), nn.Dropout1d(0.2), nn.ReLU())
        self.conv5 = nn.Sequential(nn.Conv1d(64, 32, 5, stride=1, padding=2), nn.AdaptiveMaxPool1d(output_size=64), nn.ReLU())
        self.conv6 = nn.Sequential(nn.Conv1d(32, 16, 5, stride=1, padding=2), nn.AdaptiveMaxPool1d(output_size=32), nn.ReLU())
    ...
    def forward(self, x):
        ...
        x = self.conv3(x) # 32, 100, 176
        x = self.conv4(x) # 32, 100, 176
        x = self.conv5(x) # 32, 100, 176
        x = self.conv6(x) # 32, 100, 176
...
def train(epoch, model_filename, log_filename):
    ...
    # 打印信息
    print("[EPOCH:] %s" % str(epoch + 1), "训练精度=%s" % (str(epoch_acc.item() * 100)[:5]) + '%', "最高精度=%s" % (str(best_acc.item() * 100)[:5]) + '%')

    with open(log_filename, 'a', encoding='utf-8') as f:
        f.write('[EPOCH:] {} precision={}\t'.format(epoch + 1, epoch_acc.item()))
...
def test(epoch, log_filename):
    model.eval()
    correct = 0
    test_bar = tqdm(test_loader, ncols=80)  # 形成进度条
    ...
    with open(log_filename, 'a', encoding='utf-8') as f:
        f.write('[TEST:] {} precision={}\n'.format(epoch + 1, correct.item()/len(test_dataset)))

    print('测试集准确率：{}/{}={:.2%}'.format(correct.item(), len(test_dataset), correct.item()/len(test_dataset)))
    # correct=0
    ...
    #     correct+=(pred==y).sum()
    #     break
    # print('训练集准确率：{}/{}={:.2%}'.format(correct.item(), len(train_dataset), correct.item()/len(train_dataset)))
